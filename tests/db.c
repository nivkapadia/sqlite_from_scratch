...
int main(int argc, char *argv[])
{
	if (argc < 2)
	{
		printf("Must suppy a database filename.\n");
		exit(EXIT_FAILURE);
	}
	...
	while (true)
	{
		...
		switch (prepare_statement(input_buffer, &statement))
		{
			...
			case (PREPARE_NEGATIVE_ID):
				printf("ID can not be negative. Please enter a positive id\n");
				continue;
			case (PREPARE_STRING_TOO_LONG):
				puts("The string entered is too long");
				continue;
...
ExecuteResult execute_insert(Statement *statement, Table *table)
{
	...
	table->num_rows++;
	table->pager->file_length += ROW_SIZE;

	puts("I've malfunctioned at execute_insert");
...
ExecuteResult execute_select(Statement *statement, Table *table)
{
	...
	puts("I've malfunctioned at execute_select");
...
void *get_page(Pager *pager, uint32_t page_num)
{
	...
	if (pager->pages[page_num] == NULL)
	{
		...
		if (page == NULL)
		{
			printf("Page memory allocation failed.\n");
			exit(EXIT_FAILURE);
		}
